...
- `log-bucket`：tag 里写了 `linkedTo: dataBucket.bucket`，引用了 `data` 的 Output → 与 `data` 形成**隐式依赖**，Pulumi 保证 `data` 先就绪；
- `audit-bucket`：与 `log` 没有任何数据引用，但加了 `dependsOn: [logBucket]` → **显式依赖**，强制在 `log` 之后创建。
...
你会看到 `log-bucket` 的 `dependencies` 含有 `data-bucket` 的 URN，`audit-bucket` 的 `dependencies` 含有 `log-bucket` 的 URN。
