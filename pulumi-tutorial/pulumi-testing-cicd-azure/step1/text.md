...
现在写一个 Pulumi mock 单元测试。它在导入程序前设置 mocks，然后断言 Resource Group 必须包含 owner 与 managedBy 标签，并且 Virtual Network 必须使用约定地址空间。测试代码已经由初始化脚本放在 asserts 目录中，这里先查看内容，再复制到测试目录。
