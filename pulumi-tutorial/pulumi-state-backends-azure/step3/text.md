...
上面的命令只取消了当前 shell 里的 PULUMI_BACKEND_URL。因为刚才已经把同一个 Azure Blob Backend URL 写进了 Pulumi.yaml 的 backend.url，Pulumi 会从项目文件继续读取这个地址；而 step1 的 login 已经把访问这个 Backend 所需的登录记录写入了本机 Pulumi 凭据文件。
...
同一个 Backend 中可以有多个 Stack。新建的 DIY Backend 默认按 Project 组织 Stack，所以 dev 与 prod 都会放在 state-backends-azure 这个项目路径下。
