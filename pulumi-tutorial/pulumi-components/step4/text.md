...
假设我们想把组件里的日志桶从 `${name}-logs` 改名为 `${name}-access-logs`。先看「天真」的改法——直接改名、不加 alias：

```bash
cd /root/workspace && cp variants/evolve-broken.ts index.ts && pulumi preview
```{{exec}}

注意 preview 的计划：它不是温和的 update，而是 **delete 旧的 `-logs` 桶 + create 新的 `-access-logs` 桶**。原因正是上一章那条规则——子资源的 URN 包含父组件的类型和名字，改了子资源的 logical name 就改了 URN，Pulumi 把它当成两个不相关的资源。在组件内部改名，一样会触发替换。
...
这次不再有 delete + create，而是把 state 里那条 `-logs` 记录**改名**为 `-access-logs`，云上的桶原地保留。应用它：
