...
```bash
find /root/repos/azure-secure-storage-source-work -maxdepth 1 -type f | sort
```{{exec}}

再看插件清单和语言清单：

```bash
cd /root/repos/azure-secure-storage-source-work && \
...
```bash
git -C /root/repos/azure-secure-storage-source-work log --oneline --decorate -1
```{{exec}}
