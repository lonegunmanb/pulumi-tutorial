...
Killercoda 环境的 CPU、内存和网络都比较受限，现场编译带 Azure Go SDK 的 provider 会非常慢。因此本实验把 Linux amd64 插件归档预先放在 assets 中，进入环境时解压到 provider 目录。归档保留了二进制的执行权限，所以不需要再执行 chmod。
...
```bash
tar -tzf /root/pulumi-resource-azure-secure-exec-v0.1.0-linux-amd64.tar.gz && \
ls -l /root/repos/azure-secure-exec-provider/bin/pulumi-resource-azure-secure-exec && \
pulumi plugin install resource azure-secure-exec 0.1.0 --file /root/repos/azure-secure-exec-provider/bin/pulumi-resource-azure-secure-exec --reinstall
```{{exec}}
