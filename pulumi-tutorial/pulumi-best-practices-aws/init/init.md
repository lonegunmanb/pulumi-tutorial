# 最佳实践（AWS / LocalStack RDS）

本实验用真实的 `@pulumi/aws` provider 对接本地 LocalStack。LocalStack 会模拟 AWS RDS 控制面，全程无需真实 AWS 账号。
...
环境正在后台准备（安装 Pulumi、Node.js，安装依赖，并启动 LocalStack RDS），请稍候。