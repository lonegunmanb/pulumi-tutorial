...
注意 concurrency 的作用：同一个 Pull Request 只保留最新一次 preview，避免旧提交上的结果覆盖新提交。另一个名为 pulumi-preview-act 的工作流只给本地 act 模拟使用，它复用当前已经启动的 MiniStack，不再声明 service container。