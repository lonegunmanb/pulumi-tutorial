...
最后用 `act` 在本地真实执行一次 PR preview 工作流。GitHub Actions 正式工作流使用 service container；act 在本实验环境中对 service container 的处理不稳定，所以这里使用专门的本地模拟工作流，它复用后台已经启动的 MiniStack，但仍然通过 act 执行 workflow 里的步骤。
