...
这就是命名空间隔离的价值。组件作者用 `new pulumi.Config("app")` 圈出 `app:` 这块键空间，宿主项目用默认的 `new pulumi.Config()` 圈出 `pulumi-config:` 这块；即便两边都有名为 `bucketPrefix` 的键，存进同一份 `Pulumi.dev.yaml` 也不会打架。可复用组件正是靠这一点，做到「自带配置、即插即用」。
