...
本章实验只配置 Azure 版，使用本地后端和 `miniblue` 模拟 Azure 风格资源。你会创建 `dev` 与 `prod` 两个 Stack，观察 active stack、配置文件、输出、状态导出，并亲眼看到“资源名依赖 Stack 名”的 Stack 改名会改写资源（模拟环境是 update，真实云通常是 replace），再对比空 Stack 的 rename / rm 有多安全。
...
- 一次“有资源 Stack 改名会改写资源”的演示，以及一次空 Stack rename / rm 的安全对照。