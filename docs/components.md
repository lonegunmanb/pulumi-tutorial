...
组件可以存在于代码能到达的任何地方：直接写在 Pulumi 程序里、通过语言生态的库（npm / PyPI / NuGet…）分享，或打包成 **Pulumi Package** 跨语言复用。本章聚焦最常见的「直接写在程序里」的本地组件，发布与打包在 [Pulumi Packages 与 Policy as Code](packages-crossguard.md) 一章展开。
...
> 怎么把自己的组件**发布**成上面的 Package（加 `PulumiPlugin.yaml`、语言清单、入口文件），是 [Pulumi Packages 与 Policy as Code](packages-crossguard.md) 一章的主题。
