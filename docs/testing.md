...
![Pulumi 测试金字塔：从快速单元测试到少量端到端检查](./images/testing-pyramid.png)

> 绘图提示词：淡水彩阴影漫画插画风格（light watercolor shaded comic illustration），青色（cyan）主色调，拟物质感。画面是一座四层透明玻璃测试塔，最底层宽大写 unit tests / mocks，第二层写 preview + Policy Pack / property tests，第三层写 integration tests / temporary stack，顶部小平台写 end-to-end checks；旁边一只 Pulumi mascot platypus，青色身体，嘴巴颜色是 #F361D6，圆圆的眼睛，短短的四肢，憨态可掬，拿着检查清单把小积木逐层放上去；背景有 CI/CD conveyor belt、无品牌齿轮和云资源小模型，不出现真实厂商 logo；整体强调反馈速度从快到慢、真实度从低到高。
