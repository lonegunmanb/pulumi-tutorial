...
2. 它的初始值会作为 **secret 写进 Pulumi state 的 inputs**——但**永远不会出现在 state outputs 里**；
3. 后续的 **Read 操作**中，这个值会被置为 `null`；
4. 后续的 **preview / update** 中，Pulumi **不会**对这些字段检测或显示 diff——因为它们根本没被记录在 state 里供比对。

把这四条和 secret 对比着看，差异就清楚了：

| 维度 | Secret | 只写字段（write-only） |
|------|--------|------------------------|
| 是否进 state | 加密后进 state（inputs 与 outputs 都可能有） | 仅作为 secret 进 **inputs**，**outputs 永远没有** |
| 能否读回 | 能（解密后读回） | **不能**，Read 后为 `null` |
...
> 本章动手实验用的是 AWS **Secrets Manager** 的 `aws.secretsmanager.SecretVersion`，它的只写字段是 `secretStringWo`，配套版本字段是 `secretStringWoVersion`，机制与上面的 SSM 例子完全一致：只改 `secretStringWo` 而不动 `secretStringWoVersion`，Pulumi 不会触发更新；要让新值生效，必须把 `secretStringWoVersion` 递增。
...
- **只写字段 ≠ secret**：只写字段只进 inputs（作为 secret）、outputs 永远为空、不参与普通 diff，靠版本字段触发更新；它主要为对齐 Terraform schema 而存在，**能用 secret 就优先用 secret**。
